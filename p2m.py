...
#code to compare the xy=k and xy=l^2 methods and xy/N=l^2
import numpy as np
import random as rd
...
fig.update_yaxes(title_text='t/e',row=1)    

fig.add_trace(go.Scatter(x=steps, y=em, name='ETH in market'), row=2,col=1)
fig.update_xaxes(title_text='Time Steps',row=2)
fig.update_yaxes(title_text='t/e',row=2)  

fig.add_trace(go.Scatter(x=steps, y=k/e**2, name='dai/eth pool'), row=3,col=1)
fig.add_trace(go.Scatter(x=steps, y=km/em**2, name='dai/eth market'), row=3,col=1)
fig.update_xaxes(title_text='Time Steps',row=3)
fig.update_yaxes(title_text='l',row=3) 

fig.add_trace(go.Scatter(x=steps, y=e*tm/em + t, name='pool holdings in ETH'), row=4,col=1)
